...
- Circle fill: very low-opacity tint — `rgba(27,42,74,0.04)` for ink set, `rgba(79,93,117,0.05)` for muted. Tints compound naturally in overlap regions.
...
- **Set labels** placed outside the circle, NEVER crossing the stroke. Inter 12–14px 600 for the set name, optional Geist Mono 9px sublabel.
- **Intersection labels** placed inside the overlap region, Inter 12px 600, centered. For small overlaps, use a leader line to a label in clear space.
- **Accent accent** on the ONE focal intersection — the "sweet spot". Either accent label stroke OR clipPath-bounded accent fill tint (`rgba(61,107,229,0.10)`).
