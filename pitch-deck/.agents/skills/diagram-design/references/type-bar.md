...
- **Y-axis gridlines:** 4–6 horizontal lines at regular intervals. Stroke `rgba(27,42,74,0.08)` (very faint), 0.8px. X-axis baseline at `rgba(27,42,74,0.25)`, 1px.
- **Y-axis labels:** right-aligned Geist Mono 8px muted, at x=72 (8px left of the plot area).
- **X-axis labels:** centered below each bar, Inter sans 11px 600 for category names.
...
<!-- Bar body -->
<rect x="X" y="Y" width="W" height="H" fill="rgba(79,93,117,0.15)" stroke="#6b7789" stroke-width="1"/>
<!-- Value label above bar -->
<text x="X+W/2" y="Y-8" fill="#6b7789" font-size="8" font-family="'Geist Mono', monospace" text-anchor="middle">VALUE</text>
```

Focal bar: replace fill with `rgba(61,107,229,0.12)`, stroke with `#3d6be5`, label fill with `#3d6be5`.
...
- **Plot area:** left margin 200px for row labels — this replaces the 80px margin above — so `x` 200→960, `y` 40→420 inside `0 0 1000 500`. Row labels right-aligned at x=188, Inter 11px 600 ink (the category-label size this type already uses).
...
- **Gridlines:** vertical at each tick, `rgba(27,42,74,0.08)` 0.8px, spanning y 56→408. At the domain floor the axis line replaces the gridline rather than doubling it: `rgba(27,42,74,0.25)` 1px, y 40→420.
...
- **Connector:** `rgba(27,42,74,0.55)` 1px, declared before both dots so the dots cap it. It is not the axis hairline: the connector is what says *these two dots are one row*, so it has to clear 3:1 (0.55 gives 3.19:1; the 0.25 the axis uses gives 1.60:1).
...
<line x1="458" y1="96" x2="740" y2="96" stroke="rgba(27,42,74,0.55)" stroke-width="1"/>
<circle cx="458" cy="96" r="6" fill="#fafbfc" stroke="#6b7789" stroke-width="1.5"/>
<circle cx="740" cy="96" r="6" fill="#3d6be5" stroke="#1b2a4a" stroke-width="1"/>
<text x="446" y="100" fill="#6b7789" font-size="8" font-family="'Geist Mono', monospace" text-anchor="end">34</text>
<text x="752" y="100" fill="#6b7789" font-size="8" font-family="'Geist Mono', monospace">71</text>
<text x="188" y="100" fill="#1b2a4a" font-size="11" font-weight="600" font-family="'Inter', sans-serif" text-anchor="end">Platform</text>
```
...
**Dark theme.** Dots and labels swap as expected — hollow fill `#1b2a4a` with `#d8dee8` stroke, solid `#7fa0f0` with an `#fafbfc` stroke, labels `#d8dee8`. **The hairlines must invert too:** gridlines `rgba(233,237,244,0.08)`, axis `rgba(233,237,244,0.20)`, connector `rgba(233,237,244,0.40)` — the connector again heavier than the axis, for the same 3:1 reason. `rgba(27,42,74,…)` *is* the dark paper colour at every alpha value, so a connector, gridline, or axis carried over from light composites to exactly 1.000:1 — the gap encoding and the scale both vanish.
