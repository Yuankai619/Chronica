...
A **layout variant** of the standard quadrant — same house skin (warm paper, dot pattern, Inter title, Inter mono eyebrows, accent focal rule). The grammar shifts: axes hold a **range** rather than a measurement; cells hold **named scenarios** rather than positioned items.
...
- **Axis lines**: `ink` (`#1b2a4a`), `stroke-width: 1.2`, `marker-start` + `marker-end` both pointing outward.
- **Focal quadrant tint**: `rgba(61,107,229,0.04)` full rect behind the focal cell.
...
- **Cell title**: Inter sans, 16px, weight 600, `ink`.
- **Cell description**: Inter sans, 11px, `muted`, 1–3 lines, left-aligned inside the cell.
...
- Sans-serif H1 — keep Inter for the page title. The title/diagram contrast is the house signature.
