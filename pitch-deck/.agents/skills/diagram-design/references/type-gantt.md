...
- **Left label column:** x=20–200 (180px). Task names in Inter sans 11px 600. Phase labels as Geist Mono 7px eyebrows above each group.
...
- **Phase grouping:** a subtle zone rect (same pattern as architecture zone) behind each phase's rows, with an eyebrow label in the top-left margin. Use `rgba(27,42,74,0.02)` fill, `rgba(27,42,74,0.10)` stroke.
...
<rect x="X_start" y="ROW_Y+8" width="DURATION_PX" height="24" rx="4"
      fill="rgba(79,93,117,0.15)" stroke="#6b7789" stroke-width="1"/>
<text x="X_start+8" y="ROW_Y+25" fill="#1b2a4a" font-size="10" font-weight="600"
      font-family="'Inter', sans-serif">Task name</text>

<!-- Focal task -->
<rect x="X_start" y="ROW_Y+8" width="DURATION_PX" height="24" rx="4"
      fill="rgba(61,107,229,0.12)" stroke="#3d6be5" stroke-width="1"/>
<text x="X_start+8" y="ROW_Y+25" fill="#3d6be5" font-size="10" font-weight="600"
      font-family="'Inter', sans-serif">Key task</text>
```
