...
If the site has only one family, keep the schematic defaults for the missing roles (Inter for title, Geist Mono for mono). Don't force-pick a mono font that isn't on the site.
...
**From Markdown tables:** look for rows with hex values (`#rrggbb`) adjacent to role-like words. A row like `| accent | #3d6be5 |` maps directly.
...
```json
{ "colors": { "brand": { "primary": { "value": "#3d6be5", "type": "color" } } } }
```
