...
focal_accent: "#3d6be5"             # one color for all focal components (default = SKILL accent)
dark: false
```
...
Bars span the full zone width minus 16-px padding on each side. Bars marked `focal: true` use `bar_h_focal=56` and accent styling (fill `rgba(focal_accent, 0.08)`, stroke `focal_accent`). Non-focal bars use `bar_h_default=44` with muted styling (fill `rgba(27,42,74,0.05)`, stroke `rgba(27,42,74,0.32)`).
...
All side-column nodes use fixed `w=160 h=64`. Same fill / stroke pattern: fill `rgba(79,93,117,0.06)`, stroke `#8a94a6`, stroke-width 1.
...
|---|---|---|---|---|---|
| `primary` | `#3d6be5` (focal_accent) | 1.4 | — | `arrow-accent` | Every edge whose endpoint is a `focal: true` component. Also every Trino → consumer edge (serve-flow rule). |
| `secondary` | `#6b7789` (muted) | 1.2 | — | `arrow` | Default for internal platform-component edges and source → platform edges that don't touch focal. |
| `federated` | `#127c71` (link-blue) | 1.0 | `4,3` | `arrow-link` | Federation queries (e.g., source DB → Trino). |
| `trigger` | `#6b7789` (muted) | 1.0 | `4,3` | `arrow` | Every edge originating from a `kind: bar` component (Airflow drops). Unlabelled. |
| `auth` | `#3d6be5` | 1.2 | `5,4` | `arrow-accent` | Every edge from a footer node up to the zone bottom edge. **Never to a specific component.** |
...
<defs>
  ...
  <marker id="arrow-link"   markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#127c71"/></marker>
...
| Token | Light | Dark |
|---|---|---|
| Page paper | `#fafbfc` | `#1b2a4a` |
| Ink | `#1b2a4a` | `#fafbfc` |
| Muted | `#6b7789` | `#d8dee8` |
| Accent | `#3d6be5` | `#7fa0f0` |
| Link (federated) | `#127c71` | `#4fb3a5` |
| Side-column fill | `rgba(79,93,117,0.06)` | `rgba(233,237,244,0.06)` |
| Side-column stroke | `#8a94a6` | `rgba(233,237,244,0.30)` |
| Zone fill | `rgba(27,42,74,0.025)` | `rgba(233,237,244,0.04)` |
| Zone stroke | `rgba(27,42,74,0.32)` | `rgba(233,237,244,0.30)` |
| Non-focal bar fill | `rgba(27,42,74,0.05)` | `rgba(233,237,244,0.06)` |
