...
- Dot pattern: 22×22 grid, `circle r=0.8`, `fill rgba(27,42,74,0.10)`. Opacity 0.55.
- Alternating lane tints: odd-indexed lanes (0, 2, …) receive `rgba(27,42,74,0.018)` fill from `x=140` to `viewBox_w`.
- Lane dividers: horizontal hairlines at every `lane_y_top(k)` and at `legend_y_top`; stroke `rgba(27,42,74,0.12)` width 0.8.
- Label column right border: vertical hairline at `x = label_col_w`, stroke `rgba(27,42,74,0.20)` width 1, from `y = header_h` to `y = legend_y_top`.
...
- Default fill: `rgba(27,42,74,0.12)`, number text ink.
- Focal fill: `rgba(61,107,229,0.20)`, number text accent (§5).
...
- Default fill: muted (`#6b7789` light / `#d8dee8` dark).
- Focal fill: accent (`#3d6be5` light / `#7fa0f0` dark).
...
|---|---|---|---|---|---|
| `normal` | `#6b7789` (muted) | 1.0 | — | `arrow` | Standard data hand-off between steps or actors. Unlabelled. |
| `focal-in` / `focal-out` | `#3d6be5` (accent) | 1.2 | — | `arrow-accent` | Every edge whose endpoint is the focal node (`focal-in`) or origin is the focal node (`focal-out`). |
| `trigger` | `#6b7789` (muted) | 1.0 | `4,3` | `arrow-sm` | Orchestration trigger (scheduler → tool, manual override → upstream step). Unlabelled. |
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="11" cy="11" r="0.8" fill="rgba(27,42,74,0.10)"/>
  </pattern>
...
| Lane tint | `rgba(27,42,74,0.018)` | `rgba(233,237,244,0.025)` |
| Dividers | `rgba(27,42,74,0.12)` | `rgba(233,237,244,0.12)` |
| Label col divider | `rgba(27,42,74,0.20)` | `rgba(233,237,244,0.22)` |
| Default chip fill | `rgba(27,42,74,0.12)` | `rgba(233,237,244,0.12)` |
| Focal chip fill | `rgba(61,107,229,0.20)` | `rgba(127,160,240,0.22)` |
| Default node fill | white | `rgba(233,237,244,0.04)` |
| Default node stroke | `rgba(27,42,74,0.25)` | `rgba(233,237,244,0.20)` |
| Focal node fill | `rgba(61,107,229,0.08)` | `rgba(127,160,240,0.12)` |
| Focal node stroke | `#3d6be5` | `#7fa0f0` |
...
|------|---------------|--------------|---------|
| `LS` | `#5f7a8c` sage | `#8aa3b4` | List / assignment / task |
| `DB` | `#7c8f6f` dusty-blue | `#9caf8f` | Dataset / tabular records |
