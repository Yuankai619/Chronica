...
Open [`references/style-guide.md`](references/style-guide.md) and check the default tokens. If they're still the shipped defaults (paper `#fafbfc`, ink `#1b2a4a`, accent `#3d6be5` atomic-tangerine), **pause and ask the user**:
...
| JetBrains Mono as blanket "dev" font | Mono is for *technical* content — ports, commands, URLs. Names go in Inter sans. |
...
| Accent on every "important" node | The accent marks 1–2 editorial focal points, not a signaling system |
...
**The design system is skinnable.** All colors, typography, and tokens live in a single source of truth — [`references/style-guide.md`](references/style-guide.md). This file describes semantic roles (`paper`, `ink`, `muted`, `accent`, `link`, …). This install is already skinned to the `indigo-brief` slide theme (cool-white paper, deep-indigo ink, steel-blue accent, slate-grey muted); the onboarding gate does not apply here. To change the skin, edit `style-guide.md` and `themes/indigo-brief.md` together.
...
**Focal rule:** `accent` (steel-blue) goes on 1–2 elements max. Everything else is `ink` / `muted` / `soft`. If you're tempted to accent 4 things, you haven't decided what's focal yet.
...
- **Title** — Inter + Noto Sans TC, 1.75rem, 700 — H1 only
...
- **Editorial aside** — Inter + Noto Sans TC, 14px, `muted` — callouts only, never italic (Chinese has no true italic)

**Mono is for technical content only** — never as a blanket "dev" font, never JetBrains Mono, and never for Chinese (Geist Mono has no CJK coverage).
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="1" cy="1" r="0.9" fill="rgba(27,42,74,0.10)"/>
  </pattern>
</defs>
<rect width="100%" height="100%" fill="#fafbfc"/>
...
<marker id="arrow" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#6b7789"/>
</marker>
<marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#3d6be5"/>
</marker>
<marker id="arrow-link" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#127c71"/>
</marker>
```

| Arrow | Stroke | When |
|---|---|---|
| Default | muted `#6b7789` | Internal, generic |
| Accent | steel-blue `#3d6be5` | Primary / highlighted / headline |
| Link-teal | `#127c71` | HTTP/API calls, external systems |
...
<rect x="X" y="Y" width="W" height="H" rx="6" fill="#fafbfc"/>
<!-- 2. Styled box -->
...
<!-- 4. Node name (Inter sans — human-readable) -->
<text x="CX" y="CY+2" fill="#1b2a4a" font-size="12" font-weight="600"
      font-family="'Inter', sans-serif" text-anchor="middle">Node Name</text>
<!-- 5. Technical sublabel (Geist Mono) -->
<text x="CX" y="CY+18" fill="#6b7789" font-size="9"
...
<rect x="MID_X-18" y="ARROW_Y-20" width="36" height="12" rx="2" fill="#fafbfc"/>
<text x="MID_X" y="ARROW_Y-11" fill="#8a94a6" font-size="8"
...
<line x1="30" y1="LEGEND_Y-8" x2="VIEWBOX_W-30" y2="LEGEND_Y-8"
      stroke="rgba(27,42,74,0.10)" stroke-width="0.8"/>
<text x="30" y="LEGEND_Y+8" fill="#6b7789" font-size="8" font-family="'Geist Mono', monospace"
...
1. **Header** — eyebrow (Geist Mono), title (Inter), optional subtitle (Inter muted).
...
- [ ] Every arrow label has an opaque `fill="#fafbfc"` rect behind it?
...
- [ ] Human-readable names in Inter sans, not Geist Mono?
- [ ] Technical sublabels (ports, commands, URLs) in Geist Mono?
- [ ] Page title in Inter?
- [ ] Annotation callouts (if any) in *italic* Inter? (see [primitive-annotation.md](references/primitive-annotation.md))
